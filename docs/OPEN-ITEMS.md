...
- [ ] **Adopt the Linux baseline** — the Ansible role now exists (`ansible/linux-baseline/`, implements [linux-baseline.md](conventions/linux-baseline.md)). Remaining: run it across the fleet (`--check` first), reference it from each service runbook (replace the copy-pasted "Base config"), and roll out Level 2 hardening to edge/bastion hosts first — [#25]
